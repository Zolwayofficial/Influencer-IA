...
let autonomousModeEnabled = false; // modo autónomo: vision + respuestas automáticas
...
async function processChat(msg) {
  ...
  try {
    // Modo autónomo + screenshot reciente (<30s) → usar vision model
...
// Screenshot del screen share — recibido desde el avatar frontend cada 10s
...
app.get('/api/status', (_req, res) => {
  res.json({ autoShowcaseEnabled, autonomousModeEnabled, historySize: conversationHistory.length });
});
